...
func handleCheckpointsForLevel(log logr.Logger, details *checkpointDetails, level string, policy Policy) {
	...
	if policy.MaxCheckpoints > 0 && checkpointArchivesCounter > policy.MaxCheckpoints {
		...
		toDelete := selectArchivesToDelete(filteredArchives, archiveSizes, excessCount, ByCount)
	...
	if policy.MaxTotalSize > 0 && totalSize > int64(policy.MaxTotalSize) {
		...
		toDelete := selectArchivesToDelete(filteredArchives, archiveSizes, excessSize, BySize)
		for _, archive := range toDelete {
...
func selectArchivesToDelete(archives []string, archiveSizes map[string]int64, excess int64, policy RetentionPolicy) []string {
